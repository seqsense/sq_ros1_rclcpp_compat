...
public:
  ...
  explicit ParameterValue(const std::vector<int64_t> & v) : type_(TYPE_INT_ARRAY), int_array_val_(v)
  {
  }
...
template <>
inline bool ParameterValue::get<bool>() const
{
  return bool_val_;
}
template <>
inline int ParameterValue::get<int>() const
{
  return static_cast<int>(int_val_);
}
template <>
inline int64_t ParameterValue::get<int64_t>() const
{
  return int_val_;
}
template <>
inline double ParameterValue::get<double>() const
{
  return double_val_;
}
template <>
inline std::string ParameterValue::get<std::string>() const
{
  return string_val_;
}
template <>
...
public:
  Parameter() = default;
  Parameter(const std::string & name, const ParameterValue & value) : name_(name), value_(value) {}
...
public:
  ...
  : name_(node_name), nh_("~"), logger_(rclcpp::get_logger(node_name))
  {
  }
