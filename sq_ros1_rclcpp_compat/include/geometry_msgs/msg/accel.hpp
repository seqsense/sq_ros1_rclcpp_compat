#ifndef GEOMETRY_MSGS__MSG__ACCEL_HPP_
#define GEOMETRY_MSGS__MSG__ACCEL_HPP_

// Auto-generated ROS 2-style compatibility header for ROS 1.
// Provides: geometry_msgs::msg::Accel as a using-alias for the ROS 1
// type geometry_msgs::Accel. Logic-layer code that declares shared_ptr
// arguments as std::shared_ptr<const T> works on both builds; on ROS 1
// the IF layer converts boost::shared_ptr -> std::shared_ptr via
// sq_ros1_compat::to_std() (no deep copy).

#include "geometry_msgs/Accel.h"

namespace geometry_msgs {
namespace msg {
using Accel = ::geometry_msgs::Accel;
}  // namespace msg
}  // namespace geometry_msgs

#endif  // GEOMETRY_MSGS__MSG__ACCEL_HPP_
